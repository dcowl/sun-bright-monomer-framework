...
public class DateUtils {
     ...
     * @param num     数量，前为负数，后为正数
     * @param type    类型，可参考Calendar的常量(如：Calendar.HOUR、Calendar.MINUTE、Calendar.SECOND)
     * @return String
     */
    public static String getDate(String pattern, int num, int type) {
    ...
    public static double getDistanceOfTwoDate(Date before, Date after) {
        double beforeTime = (double) before.getTime();
        double afterTime = (double) after.getTime();
     ...
     * @return 天数
     */
    public static int getMonthHasDays(Date date, String format) {
        String yyyyMM = FastDateFormat.getInstance(format).format(date);
        ...
        if (day31.contains(month)) {
            ...
            boolean tag = (y % 4 == 0 && (y % 100 != 0)) || y % 400 == 0;
            if (tag) {
                day = 29;
            } else {
                day = 28;
